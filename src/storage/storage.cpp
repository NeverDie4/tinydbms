...
bool read_metadata(std::ifstream& input, std::vector<TableMeta>& tables) {
    std::string line;
...
OpenStorageResult open_storage(const OpenStorageRequest& request) {
    try {
        ...
        if (metadata_exists) {
            ...
            if (!read_metadata(input, state.tables)) {
                const bool io_failed=input.bad();
                state.data_dir.clear();
                return {error(io_failed ? StorageErrorKind::kIoError : StorageErrorKind::kCorrupt,
                              io_failed ? "cannot read storage.meta" : "invalid storage.meta")};
            }
        } else {
...
ScanNextResult scan_next(CursorId id) {
    ...
    auto result=state.cursors->next_record(id);
    return {std::move(result.value),data_error(result.error)};
}

std::optional<StorageError> close_cursor(CursorId id) {
    if(auto e=require_data_open(true))return e;
    return data_error(state.cursors->close(id));
}
...
DeleteResult delete_records(const DeleteRequest& request) {
    ...
    if(request.record_ids.empty())return {};
    if(state.cursors->has_cursor(request.table_id))
        return {0,error(StorageErrorKind::kInvalidRequest,"table has an unclosed cursor")};
    internal::HeapTable heap(*table,*state.file_manager,*state.buffer_pool);
    auto result=heap.delete_batch(request.record_ids);
