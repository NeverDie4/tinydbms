...
static_assert(sizeof(float) == 4, "tinydbms disk format requires 32-bit float");
static_assert(sizeof(double) == 8, "tinydbms disk format requires 64-bit double");
static_assert(std::numeric_limits<float>::is_iec559,
              "tinydbms disk format requires IEC 559 float");
static_assert(std::numeric_limits<double>::is_iec559,
              "tinydbms disk format requires IEC 559 double");
...
RecordCodecResult<MeasuredSize> measure(const TableMeta& table,
    ...
    for (std::size_t index = 0; index < values.size(); ++index) {
        ...
        if (!value_matches_type(type, value)) {
            return failure<MeasuredSize>(RecordCodecErrorKind::kInvalidArgument,
...
RecordCodecResult<std::vector<std::byte>> RecordCodec::encode(
    ...
    for (std::size_t index = 0; index < values.size(); ++index) {
        ...
        switch (type) {
            ...
            case Type::kFloat:
                append_little_endian(output,
                    std::bit_cast<std::uint32_t>(std::get<float>(value.data)));
                break;
            case Type::kDouble:
                append_little_endian(output,
    ...
    for (const ColumnMeta& column : table.columns) {
        switch (column.type) {
            ...
            case Type::kFloat: {
                ...
                values.push_back(Value{std::bit_cast<float>(bits)});
                logical_size += 4;
                break;
            }
            case Type::kDouble: {
                ...
                values.push_back(Value{std::bit_cast<double>(bits)});
                logical_size += 8;
                break;
            }
            case Type::kBool:
                if (offset >= payload.size() ||
                    (payload[offset] != std::byte{0} && payload[offset] != std::byte{1})) {
                    return failure<std::vector<Value>>(RecordCodecErrorKind::kCorrupt,
                                                       "invalid BOOL payload");
                }
                values.push_back(Value{payload[offset++] == std::byte{1}});
                logical_size += 1;
                break;
            case Type::kVarchar: {
