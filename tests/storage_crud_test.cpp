...
void create(TableId id=2){
    const auto result=create_table({id,id==2?"records":"other",{{"s",Type::kVarchar}}});
    check(!result.error);
}
std::vector<Value> row(std::size_t n=10,char c='x'){return {{std::string(n,c)}};}
std::vector<Record> scan(TableId id=2){auto opened=open_table({id});check(opened.cursor && !opened.error);
...
void e2e(){
    ...
    auto first=open_table({2});check(first.cursor && *first.cursor==0);check(!close_cursor({*first.cursor}).error);
    kind(insert({2,{}}).error,StorageErrorKind::kInvalidRequest);check(!delete_records({2,{}}).error);check(scan().empty());
    std::vector<std::vector<Value>> values;for(int i=0;i<10;++i)values.push_back(row(1024,static_cast<char>('a'+i)));
    auto added=insert({2,values});check(!added.error && added.rids.size()==10);
    ...
    auto partial=insert({2,{row(),{{std::int32_t{7}}},row()}});kind(partial.error,StorageErrorKind::kInvalidRequest);check(partial.rids.size()==1);
    kind(insert({2,{row(1025)}}).error,StorageErrorKind::kValueTooLarge);
    auto deleted=delete_records({2,{added.rids[1],added.rids[5]}});check(!deleted.error && deleted.deleted_count==2);
    auto prefix=delete_records({2,{added.rids[2],added.rids[2],added.rids[3]}});
    ...
    auto held=open_table({2});check(held.cursor.has_value());check(!close_storage({}).error);
    kind(scan_next({*held.cursor}).error,StorageErrorKind::kCursorInvalid);
    check(!open_storage({temp.path.string()}).error);
    kind(scan_next({*held.cursor}).error,StorageErrorKind::kCursorInvalid);
    auto newer=open_table({2});check(newer.cursor && *newer.cursor>*held.cursor);check(!close_cursor({*newer.cursor}).error);
    ...
    auto reused=insert({2,{row(1024)}});check(!reused.error && reused.rids.size()==1);
    kind(delete_records({2,{added.rids[1]}}).error,StorageErrorKind::kInvalidRequest);
    check(!close_storage({}).error);check(!open_storage({temp.path.string()}).error);
    kind(delete_records({2,{added.rids[1]}}).error,StorageErrorKind::kInvalidRequest);
    check(!delete_records({2,reused.rids}).error);check(!close_storage({}).error);
}
...
void restrictions(){
    Temp temp;check(StorageTestAccess::configure(1));check(!open_storage({temp.path.string()}).error);create();create(3);
    auto i=insert({2,{row(),row(20)}});check(!i.error);auto id=i.rids[0];
    auto* pool=StorageTestAccess::buffer_pool();auto fetches=pool->stats().fetch_count;
    auto a=open_table({2}),b=open_table({2});check(a.cursor && b.cursor && *b.cursor>*a.cursor);
    check(pool->stats().fetch_count==fetches);blocked(2,id);
    auto other=insert({3,{row()}});check(!other.error);check(!delete_records({3,other.rids}).error);
    ...
    check(!scan_next({*a.cursor}).record);blocked(2,id); // EOF still blocks.
    check(!close_cursor({*a.cursor}).error);blocked(2,id); // Other cursor remains.
    check(!close_cursor({*b.cursor}).error);kind(close_cursor({*b.cursor}).error,StorageErrorKind::kCursorInvalid);
    check(!insert({2,{row()}}).error);check(!delete_records({2,{id}}).error);
...
void nonopen(bool closing=false){
    kind(insert({2,{}}).error,StorageErrorKind::kInvalidRequest);
    kind(delete_records({2,{}}).error,StorageErrorKind::kInvalidRequest);
    kind(open_table({2}).error,StorageErrorKind::kInvalidRequest);
...
void lifecycle(){
    ...
    check(StorageTestAccess::configure(1,ReplacementPolicy::kFifo,
        [&](PageKey key,const RawPage& p)->std::optional<PageFileError>{
            ...
            return current_files->find_table_file(key.table_id)->write_page(key.page_id,p);}));
    check(!open_storage({temp.path.string()}).error);create();files=StorageTestAccess::file_manager();
    check(!insert({2,{row()}}).error);auto opened=open_table({2});check(opened.cursor.has_value());
    auto pin=StorageTestAccess::buffer_pool()->fetch_page({2,1});check(pin.value.has_value());
    ...
    blocked(2,RecordId{});fail=false;check(!close_storage({}).error);nonopen();
    ...
    auto c=open_table({2});check(c.cursor.has_value());StorageTestAccess::fail_next_file_close();
...
void failures(){
    for(bool corrupt:{false,true}){
        ...
        if(corrupt)check(StorageTestAccess::file_manager()->find_table_file(2)->allocate_page().value==1);
        else check(!insert({2,{row()}}).error);
        std::cerr<<"  failure b\n";check(!close_storage({}).error);std::cerr<<"  failure c\n";check(!open_storage({temp.path.string()}).error);fail=!corrupt;
        auto opened=open_table({2});check(opened.cursor.has_value());
        ...
        blocked(2,RecordId{});check(BufferPoolTestAccess::no_pins(*StorageTestAccess::buffer_pool()));
        check(BufferPoolTestAccess::clean(*StorageTestAccess::buffer_pool()));check(!close_cursor({*opened.cursor}).error);
        if(!corrupt)check(!insert({2,{row()}}).error);
        std::cerr<<"  failure e\n";check(!close_storage({}).error);
    }
    Temp temp;check(StorageTestAccess::configure(1));check(!open_storage({temp.path.string()}).error);create();
    auto data=insert({2,{row(1024),row(1024),row(1024),row(1024)}});check(!data.error);
    auto guard=StorageTestAccess::buffer_pool()->fetch_page({2,2});check(guard.value.has_value());
    auto failure=delete_records({2,{data.rids[0]}});kind(failure.error,StorageErrorKind::kInvalidRequest);
    check(failure.error->message.find("unpinned")!=std::string::npos);
    guard.value->release();check(!close_storage({}).error);
}
void holes_retired(){
    Temp temp;check(StorageTestAccess::configure(1));check(!open_storage({temp.path.string()}).error);create();
    auto* file=StorageTestAccess::file_manager()->find_table_file(2);
    check(file->allocate_page().value==1);check(file->allocate_page().value==2);check(!file->free_page(1));
    RawPage p;check(!RecordPage::initialize(p,2));TableMeta meta{2,"records",{{"s",Type::kVarchar}}};
...
void exhausted(){
    ...
    CursorRegistryTestAccess::exhaust_soon();auto last=open_table({2});
    check(last.cursor && *last.cursor==std::numeric_limits<CursorId>::max());
    kind(open_table({2}).error,StorageErrorKind::kInvalidRequest);check(!close_storage({}).error);
    check(!open_storage({temp.path.string()}).error);kind(open_table({2}).error,StorageErrorKind::kInvalidRequest);
    check(!close_storage({}).error);
}
}
