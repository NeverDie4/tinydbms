...
void run() {
    ...
    check(StorageTestAccess::configure(2,ReplacementPolicy::kLru,
        [&](PageKey key,const RawPage& page)->std::optional<PageFileError> {
            ...
            check(create_table({9,"blocked",{{"id",Type::kInt32}}}).error.has_value());
    ...
    check(!create_table({0,"zero",{{"id",Type::kInt32},{"name",Type::kVarchar}}}).error);
    ...
    TableMeta meta{0,"zero",{{"id",Type::kInt32},{"name",Type::kVarchar}}};
    ...
    check(row.value && row.value->record_id.value==record.value->value);
