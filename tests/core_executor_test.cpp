...
bool run_invalid_insert(Plan plan) {
    ...
    CHECK(script.statements.size() == 1);
    CHECK(is_error(script.statements.front(), ErrorKind::kInternal));
...
bool test_bigint_validation_and_mixed_integer_comparison() {
    ...
    CHECK(is_error(invalid.statements.front(),ErrorKind::kInternal));
    CHECK(fake::state().insert_calls==0);
...
bool test_double_physical_validation() {
    ...
    for (Value invalid : {Value{std::int32_t{1}}, Value{std::int64_t{1}}}) {
        ...
        CHECK(is_error(result.statements.front(), ErrorKind::kInternal));
        CHECK(fake::state().insert_calls == 1);
    }
    CHECK(close_database(database));
    return true;
...
bool test_insert_storage_errors_and_script_stop() {
    ...
    for (std::size_t index = 1; index < script.statements.size(); ++index) {
        CHECK(script.statements[index].status == tinydbms::core::StatementStatus::kAnalysisOnly);
        CHECK(!script.statements[index].outcome.has_value());
    }
    CHECK(script.first_error_index == std::optional<std::size_t>{0});
    ...
    CHECK(is_error(mismatch.statements.front(), ErrorKind::kInternal));
    ...
    CHECK(is_error(too_many.statements.front(), ErrorKind::kInternal));
...
bool test_non_positional_slot_plumbing_and_invalid_mappings() {
    ...
    CHECK(is_error(invalid_column_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(duplicate_column_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(duplicate_slot_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(missing_slot_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
...
bool test_query_output_metadata_must_match_project_positions() {
    ...
    CHECK(is_error(count_mismatch_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(slot_mismatch_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
...
bool test_inner_join_uses_non_positional_slots() {
    ...
    CHECK(is_error(duplicate_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    CHECK(close_database(duplicate_slot));
    ...
    CHECK(is_error(missing_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
...
bool test_sort_uses_slot_bindings_and_rejects_invalid_plans() {
    ...
    CHECK(is_error(empty_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(cross_family_result.statements.front(), ErrorKind::kInternal));
...
bool test_aggregate_uses_derived_slots_and_validates_plans() {
    ...
    CHECK(overflow_result.first_error_index == std::optional<std::size_t>{0});
    CHECK(overflow_result.executed_count == 1);
    ...
    const auto malformed = [&](AggregateNode node, std::vector<QueryOutput> outputs) {
        ...
        CHECK(is_error(invalid_result.statements.front(), ErrorKind::kInternal));
        CHECK(fake::state().open_table_calls == 0);
...
bool test_invalid_predicate_is_rejected_before_open_table() {
    ...
    CHECK(is_error(invalid_delete.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    CHECK(fake::state().delete_calls == 0);
    ...
    CHECK(is_error(invalid_query.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(invalid_varchar_order.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    CHECK(fake::state().delete_calls == 0);
    CHECK(close_database(database));
    return true;
...
bool test_invalid_query_plans_have_no_storage_side_effect() {
    ...
    CHECK(is_error(null_plan.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(null_filter_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(null_project_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(empty_project_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
    ...
    CHECK(is_error(invalid_topology_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().open_table_calls == 0);
...
bool test_open_and_scan_result_invariants_close_cursor_once() {
    ...
    CHECK(is_error(malformed_open_result.statements.front(), ErrorKind::kInternal));
    ...
    CHECK(is_error(malformed_delete_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
    CHECK(fake::state().delete_calls == 0);
    ...
    CHECK(is_error(missing_cursor_result.statements.front(), ErrorKind::kInternal));
    ...
    CHECK(is_error(malformed_scan_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
...
bool test_scan_row_and_close_failures_discard_query_and_delete() {
    ...
    CHECK(is_error(malformed_row_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
...
bool test_storage_errors_and_exceptions_are_contained() {
    ...
    CHECK(is_error(scan_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
    ...
    CHECK(is_error(post_open_table_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 0);
    ...
    CHECK(is_error(post_open_delete_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 0);
    CHECK(fake::state().close_calls == 1);
    ...
    CHECK(is_error(insert_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().insert_calls == 1);
    ...
    CHECK(is_error(delete_scan_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
    CHECK(fake::state().delete_calls == 0);
    ...
    CHECK(delete_result.statements.size() == 1);
    CHECK(is_error(delete_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().close_cursor_calls == 1);
    ...
    CHECK(is_error(post_create_result.statements.front(), ErrorKind::kInternal));
    ...
    CHECK(is_error(post_insert_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().records.size() == 1);
    CHECK(fake::state().close_calls == 1);
    ...
    CHECK(is_error(post_delete_result.statements.front(), ErrorKind::kInternal));
    CHECK(fake::state().records.empty());
    CHECK(fake::state().close_calls == 1);
