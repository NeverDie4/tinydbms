...
void scan_roundtrip() {
    ...
    for(auto id:ids){auto r=reopened.next_record(*restored.value);check(r.value && r.value->rid.value==id.value);}
...
void cursors() {
    ...
    auto first_a=registry.next_record(*a.value);check(first_a.value && first_a.value->rid.value==inserted.value->value);
    ...
    for(auto id:{*a.value,*b.value}) {
        if(id==*b.value){auto r=registry.next_record(id);check(r.value && r.value->rid.value==inserted.value->value);}
        auto r=registry.next_record(id);check(r.value && r.value->rid.value==second.value->value);
        check(BufferPoolTestAccess::unpinned_clean(*f.pool));
...
void holes_deleted_retired() {
    ...
    for(auto id:expected) {
        auto r=t.next_record(*pos.value);check(r.value && r.value->rid.value==id.value);
