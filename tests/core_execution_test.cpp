...
int main() {
    ...
    assert(!tinydbms::testing::first_error_index(created).has_value());
    assert(tinydbms::testing::executed_count(created) == 1);
    ...
    assert(oversized_error->source.has_value());
    assert(oversized_error->source->begin.line == 1 && oversized_error->source->begin.column == 1);
    ...
    // 本节验证“首错后继续静态分析 + 影子 Catalog 隔离”的恢复语义；该语义
    // 由 analyze 策略显式请求，默认 stop 策略只把后续语句记为 kSkippedExecution。
    ...
    assert(tinydbms::testing::executed_count(recovered_script) == 1);
    assert(tinydbms::testing::first_error_index(recovered_script) == std::optional<std::size_t>{1});
    ...
    assert(recovered_script.statements[1].source().begin.line == 1);
    assert(recovered_script.statements[1].source().end.line == 2);
    const Error& first_compile_error = error_of(recovered_script.statements[1]);
    assert(first_compile_error.source.has_value());
    assert(first_compile_error.source->begin.line == 2);
    assert(first_compile_error.source->begin.column == 1);
    ...
    assert(later_compile_error.source.has_value());
    assert(later_compile_error.source->begin.line == 6);
    assert(later_compile_error.source->begin.column == 1);
    ...
    assert(tinydbms::testing::first_error_index(absolute_ranges) == std::optional<std::size_t>{0});
    assert(tinydbms::testing::executed_count(absolute_ranges) == 0);
    assert(absolute_ranges.statements[0].source().begin.line == 1);
    assert(absolute_ranges.statements[0].source().begin.column == 1);
    assert(absolute_ranges.statements[0].source().end.line == 1);
    assert(absolute_ranges.statements[0].source().end.column == 35);
    assert(absolute_ranges.statements[1].source().begin.line == 1);
    assert(absolute_ranges.statements[1].source().begin.column == 35);
    assert(error_of(absolute_ranges.statements[1]).source->begin.line == 1);
    assert(error_of(absolute_ranges.statements[1]).source->begin.column == 36);
    ...
    assert(absolute_ranges.statements[2].source().begin.line == 1);
    assert(absolute_ranges.statements[2].source().begin.column == 64);
    assert(absolute_ranges.statements[2].source().end.line == 2);
    assert(error_of(absolute_ranges.statements[2]).source->begin.line == 2);
    assert(error_of(absolute_ranges.statements[2]).source->begin.column == 1);
    ...
    const auto utf8_diagnostics = database.execute_script(
        {utf8_fix_script, tinydbms::core::ScriptErrorPolicy::kAnalyzeRemaining});
    ...
    assert(utf8_error.source.has_value());
    assert(utf8_error.source->begin.line == 1);
    assert(utf8_error.source->begin.column == 43);
    ...
    const auto fixed_utf8_script = database.execute_script(
        {utf8_fix_script, tinydbms::core::ScriptErrorPolicy::kAnalyzeRemaining});
    ...
    assert(tinydbms::testing::executed_count(duplicate_shadow_create) == 0);
    ...
    assert(tinydbms::testing::executed_count(complex_shadow_analysis) == 0);
