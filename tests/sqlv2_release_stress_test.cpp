...
void verify_executed_prefix(const ExecuteScriptResult& result, std::size_t count) {
    ...
    check(tinydbms::testing::executed_count(result) == count);
    check(!tinydbms::testing::first_error_index(result).has_value());
...
void verify_recovery(
    ...
    check(tinydbms::testing::first_error_index(result) ==
          std::optional<std::size_t>{prefix_count});
    check(tinydbms::testing::executed_count(result) == prefix_count);
    check(!result.script_error.has_value());
...
void verify_real_state_and_shadow_isolation(Database& database) {
    ...
    check(tinydbms::testing::executed_count(real_state) == 3U);
...
void run_fixed_golden() {
    ...
    // 本脚本注入两条编译错误并断言其后的影子 Catalog 分析结果，因此显式请求
    // analyze 策略；默认 stop 策略下首错后的语句只会记为 kSkippedExecution。
    const ExecuteScriptResult result = database.execute_script(
        {join_script(statements), ScriptErrorPolicy::kAnalyzeRemaining});
...
void run_mixed_v1_v2_persistence() {
    ...
    check(tinydbms::testing::executed_count(mixed) == 6U);
    ...
    for (std::size_t cycle = 0; cycle < 102U; ++cycle) {
        ...
        check(tinydbms::testing::executed_count(updated) == 1U);
    }
    check(!database.close().error.has_value());
