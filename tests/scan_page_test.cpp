...
int main() try {
    ...
    const std::vector<std::byte> one{std::byte{1},std::byte{0},std::byte{0},std::byte{0}};
    const std::vector<std::byte> zero{std::byte{0},std::byte{0},std::byte{0},std::byte{0}};
    auto a=SlottedPage::insert(p,1,one); check(a.value.has_value());
    ...
    auto b=SlottedPage::insert(p,1,zero); check(b.value.has_value());
    auto c=SlottedPage::insert(p,1,one); check(c.value.has_value());
    ...
    auto before=p.bytes;
    TableMeta meta{0,"ints",{{"i",Type::kInt}}};
    auto record=RecordPage::next_record(p,1,meta,0);
    check(record.record.has_value() && !record.error && record.next_position==1);
    check(record.record->values[0].data==Value{std::int32_t{1}}.data);
    check(record.record->rid.value==RecordIdCodec::encode({1,0,1}).value->value);
    ...
    check(RecordPage::next_record(p,1,meta,1).record->rid.value==record3.record->rid.value);
    auto good=p;
    ...
    auto broken=good;
    broken.bytes[34]=std::byte{3}; // Truncate slot 0's four-byte INT payload.
    check(SlottedPage::validate(broken,1).has_value());
    ...
    p.bytes.fill(std::byte{0});check(std::get<std::int32_t>(record.record->values[0].data)==1);
