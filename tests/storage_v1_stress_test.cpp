...
std::vector<Value> row(int id,TableId table){return {{std::int32_t{id}},{std::int64_t{id}*100000},
    {float(id)+0.25F},{double(id)+0.5},{(id%2)==0},{std::string(16,table?'b':'a')}};}
...
void verify(TableId table,const Rows& expected){
    auto c=open_table({table});check(c.cursor_id && !c.error);std::set<std::uint64_t> seen;
    for(;;){auto r=scan_next(*c.cursor_id);check(!r.error);metrics();if(!r.record)break;
        auto found=expected.find(r.record->record_id.value);check(found!=expected.end());
    ...
    check(seen.size()==expected.size());check(!close_cursor(*c.cursor_id));
}
void run(std::size_t capacity,ReplacementPolicy policy,int count){
    ...
    for(TableId t=0;t<2;++t){
        check(!create_table({t,t?"second":"first",{{"i",Type::kInt32},{"l",Type::kInt64},
            {"f",Type::kFloat},{"d",Type::kDouble},{"b",Type::kBool},{"s",Type::kVarchar}}}).error);
        std::vector<std::vector<Value>> values;for(int i=0;i<count;++i)values.push_back(row(i,t));
        auto inserted=insert({t,values});check(!inserted.error && inserted.record_ids.size()==values.size());
        for(std::size_t i=0;i<values.size();++i)expected[t].emplace(inserted.record_ids[i].value,values[i]);
    ...
    for(TableId t=0;t<2;++t){
        ...
        auto inserted=insert({t,replacement});check(!inserted.error && inserted.record_ids.size()==replacement.size());
        for(std::size_t i=0;i<replacement.size();++i){auto id=inserted.record_ids[i];auto p=RecordIdCodec::decode(id);check(p.value.has_value());
    ...
    for(int cycle=0;cycle<3;++cycle){
        ...
        for(TableId t=0;t<2;++t){verify(t,expected[t]);auto extra=insert({t,{row(20000+cycle,t)}});check(!extra.error && extra.record_ids.size()==1);
            expected[t].emplace(extra.record_ids[0].value,row(20000+cycle,t));
