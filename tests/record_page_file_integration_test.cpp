...
struct Fixture {
    ...
    void expect(RecordId rid, const TableMeta& meta, const std::vector<Value>& values) {
        ...
        check(result.value.has_value()); check(result.value->record_id.value == rid.value);
        check(result.value->values.size() == values.size());
        for (std::size_t i = 0; i < values.size(); ++i) {
            check(result.value->values[i].data == values[i].data);
            if (auto f = std::get_if<float>(&values[i].data))
                check(std::bit_cast<std::uint32_t>(*f) == std::bit_cast<std::uint32_t>(std::get<float>(result.value->values[i].data)));
            if (auto d = std::get_if<double>(&values[i].data))
                check(std::bit_cast<std::uint64_t>(*d) == std::bit_cast<std::uint64_t>(std::get<double>(result.value->values[i].data)));
        }
    }
...
void persistence(const std::filesystem::path& directory) {
    ...
    TableMeta meta{0,"mixed",{{"i",Type::kInt32},{"j",Type::kInt64},
        {"f",Type::kFloat},{"d",Type::kDouble},{"b",Type::kBool},{"s",Type::kVarchar}}};
    std::vector<Value> alice{{std::int32_t{-42}},{std::int64_t{1234567890123}},
        {-0.0F},{-1.25},{true},{std::string{"Alice\xE4\xB8\xAD\xF0\x9F\x99\x82"}}};
...
void corruption(const std::filesystem::path& directory) {
    ...
    TableMeta meta{0,"payload",{{"b",Type::kBool},{"s",Type::kVarchar}}};
    auto rid = f.insert(meta,{Value{true},Value{std::string{"abc"}}});
    f.persist(); const RawPage good = f.page;
    const auto payload = u16(good,kSlottedPageHeaderSize);
    for (int case_id=0; case_id<6; ++case_id) {
        ...
        switch(case_id) {
            ...
            case 3: f.page.bytes[payload+1]=std::byte{0xff}; break;
            case 4: f.page.bytes[payload]=std::byte{2}; break;
            case 5: f.page.bytes[kSlottedPageHeaderSize+kGeneration]=std::byte{0}; break;
        }
