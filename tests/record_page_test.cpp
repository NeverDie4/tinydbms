...
int main() { try {
    ...
    TableMeta meta{0,"test",{{"i",Type::kInt32},{"j",Type::kInt64},{"f",Type::kFloat},{"d",Type::kDouble},{"b",Type::kBool},{"s",Type::kVarchar}}};
    std::vector<Value> values{{std::int32_t{-2}},{std::int64_t{9}},{1.25F},{-2.5},{true},{std::string{"\xE4\xB8\xAD\xF0\x9F\x99\x82"}}};
    ...
    check(get.value->record_id.value == a.value->value);
    ...
    auto wrong=values; wrong[0]=Value{true}; check(!RecordPage::insert_record(page,1,meta,wrong).value); check(page.bytes==snapshot);
    ...
    TableMeta boolean{0,"bool",{{"b",Type::kBool}}};
    auto bad=RecordPage::get_record(corrupt,4,boolean,*rid.value); check(bad.error && bad.error->kind==RecordPageErrorKind::kCorrupt);
    corrupt.bytes[0]=std::byte{0}; bad=RecordPage::get_record(corrupt,4,boolean,*rid.value); check(bad.error && bad.error->kind==RecordPageErrorKind::kCorrupt);
    snapshot=corrupt.bytes;
    check(RecordPage::insert_record(corrupt,4,boolean,{Value{true}}).error->kind==RecordPageErrorKind::kCorrupt);
    ...
    auto live=RecordPage::insert_record(retired,5,boolean,{Value{true}}); check(live.value.has_value());
    ...
    check(!RecordPage::get_record(retired,5,boolean,*last.value).value);
    auto next=RecordPage::insert_record(retired,5,boolean,{Value{false}});
