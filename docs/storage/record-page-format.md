...
| Type | Encoding | Bytes |
|---|---|---:|
| `kInt` | signed int32 two's-complement bit pattern, little-endian | 4 |
...
解码必须按 schema 恰好消费 Slot Entry 指定的全部 payload。short input、trailing bytes、非法 UTF-8、VARCHAR length 越界或不能按 TableMeta 完整解释均为 `kCorrupt`。
...
Phase 2C RecordCodec tests：INT/VARCHAR round-trip、字段数/类型错误、UTF-8、VARCHAR 1024 边界、logical 4096 边界、encoded 4056 边界、short input、trailing bytes、非法 VARCHAR length。
...
Phase 2E `record_page_test`：RID 位布局 golden value 与边界、INT/VARCHAR、多记录、错误类型和大小、no-space、失败不修改、wrong-page、deleted/stale/retired handle、损坏 page/payload。

Phase 2F `record_page_file_integration_test`：初始化与 INT/VARCHAR 落盘恢复、同页多记录、中间删除及 compact、删除后 reopen 再复用、generation 与 RID 跨 reopen 稳定、空页保留 slot_count、两文件隔离、magic/self PageId/flags/generation/VARCHAR 损坏、写失败报告、独立 raw page free-list 复用。
