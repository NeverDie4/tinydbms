...
enum class ScriptErrorPolicy {
    kStopOnFirstError,   // 默认：首错后剩余语句全部 kSkippedExecution
    kAnalyzeRemaining    // 首错后继续静态分析，仍不执行
};
...
struct StatementResult {
    std::size_t statement_index() const noexcept;      // 0-based，按 split 顺序且连续
    const SourceRange& source() const noexcept;        // 整段 script 的绝对范围
    StatementStatus status() const noexcept;
    const std::optional<ExecuteResult>& outcome() const noexcept;
};

struct ExecuteScriptResult {
    std::optional<Error> script_error;         // 分句失败、语句数超限或致命中止
    std::vector<StatementResult> statements;   // 覆盖全部已识别语句，index 连续
};
```
...
这是对当前 `vector<ExecuteResult> outcomes` 的有意 breaking change，但复用 ExecuteResult，避免再造结果 variant。已确认无外部消费者，采用一次性迁移：Core/App 不保留 legacy outcomes 投影视图或兼容别名，需要旧形状的调用方从 `statements` 自行派生，避免重复状态长期留在公共结构。

### 11.1 脚本错误策略

`ExecuteScriptRequest.error_policy` 默认 `kStopOnFirstError`。首个 Compile、Execution 或 Storage Error 把执行门永久置为关闭；本次调用内绝不恢复，剩余语句全部记 `kSkippedExecution`。`kAnalyzeRemaining` 下，首错之后的每个 statement 仍运行 Lexer→Parser→Semantic→Optimizer→Planner，但绝不调用 Executor 或 Storage：编译成功记 `kAnalysisOnly`（影子 Catalog 明确拒绝时记 `kAnalysisError`），编译失败仍记 `kCompileError`。两种策略下此前成功语句都保留副作用，不 rollback，本功能不是 transaction。

## 12. Analysis / Shadow Catalog

Shadow Catalog 由单次 `Database::execute_script()` 局部拥有，调用结束即丢弃。首错前 compiler 使用实时 Runtime Catalog，DDL 成功执行后照常更新它；analyze 策略的 Shadow 在调用开始即复制 Runtime Catalog 与下一候选 TableId（`std::uint64_t` 计数器，禁止窄化回 `TableId` 再递增），成功执行的 CREATE TABLE 会同步镜像到 Shadow，首错之后的 compile 只借用 Shadow Catalog。

analysis-only CREATE 编译成功且未被拒绝时，按正常 deterministic ID allocation 模拟加入 Shadow Catalog；INSERT、UPDATE、DELETE、SELECT 不改变 schema。连续 analysis-only CREATE 因而能被后续 SELECT/JOIN/GROUP BY 看见，重复 CREATE 也会得到准确 duplicate-table Semantic Error。Shadow 明确拒绝（TableId 耗尽、保留 ID、重复名称、非法元数据）时记 `kAnalysisError`，且不消耗候选 TableId。Shadow Catalog 只复制 metadata，不访问 Storage，也不修改运行时 Catalog 与运行时 `next_table_id`。

Shadow TableId 从 Runtime `next_table_id` 副本开始，按相同规则递增；不使用 UINT32_MAX sentinel、负数或特殊高位，因为 TableId 全范围合法。这些 ID 只服务本次 analysis，Plan 不执行。ID 耗尽时记 `kAnalysisError`。
...
规则继续是 1-based line、1-based UTF-8 byte column 与 0-based byte offset；CRLF 视作一个换行且 `\r` 不占列，Tab 按字节推进，端点不得位于 CRLF 中间。`StatementResult::source()` 直接采用 `SplitStatement.source`。
...
| ExecuteScriptResult | vector outcomes | per-statement status/range + script_error + 错误策略 | recovery/analysis-only | None | High | None | High |
